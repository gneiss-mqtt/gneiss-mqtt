...
pub(crate) mod testing {
    ...
    pub(crate) fn create_good_threaded_client_builder() -> ThreadedClientBuilder {
        ...
        let endpoint = std::env::var("GNEISS_MQTT_TEST_DIRECT_PLAINTEXT_HOST").unwrap();
    ...
    fn subscribe_unsubscribe_test(client: SyncClientHandle) -> GneissResult<()> {
        start_sync_client(&client)?;
    ...
    fn subscribe_unsubscribe() {
        do_good_client_test(Box::new(|builder|{
            subscribe_unsubscribe_test(builder.build())
        }));
    }
    ...
    fn verify_publish_received(event: &PublishReceivedEvent, expected_topic: &str, expected_qos: QualityOfService, expected_payload: &[u8]) {
        ...
        assert_eq!(expected_payload, publish.payload().unwrap());
    }

    fn subscribe_publish_test(client: SyncClientHandle, qos: QualityOfService) -> GneissResult<()> {
        start_sync_client(&client)?;
    ...
    #[test]
    fn subscribe_publish_qos0() {
        do_good_client_test(Box::new(|builder|{
            subscribe_publish_test(builder.build(), QualityOfService::AtMostOnce)
        }));
    }

    #[test]
    fn subscribe_publish_qos1() {
        do_good_client_test(Box::new(|builder|{
            subscribe_publish_test(builder.build(), QualityOfService::AtLeastOnce)
        }));
    }

    #[test]
    fn subscribe_publish_qos2() {
        do_good_client_test(Box::new(|builder|{
            subscribe_publish_test(builder.build(), QualityOfService::ExactlyOnce)
        }));
    }

    fn sync_will_test(base_client_builder: ThreadedClientBuilder) -> GneissResult<()> {
        ...
        let mut will_client_builder = create_good_threaded_client_builder();
        will_client_builder.with_connect_options(connect_options);

        let will_client = will_client_builder.build();
    ...
    fn connect_disconnect_cycle_session_rejoin() {
        do_good_client_test(Box::new(|builder|{
            sync_connect_disconnect_cycle_session_rejoin_test(builder.build())
        }));
    }
    ...
    fn connection_failure_invalid_endpoint() {
        ...
        assert!(connection_failure_test(builder).is_ok());
    }

    #[test]
    fn connection_failure_invalid_endpoint_http() {
        let builder = ThreadedClientBuilder::new("amazon.com", 80);

        assert!(connection_failure_test(builder).is_ok());
    }

    #[test]
    fn connection_failure_invalid_endpoint_https() {
