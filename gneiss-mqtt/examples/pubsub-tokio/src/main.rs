...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("pubsub-tokio - an example that demonstrates publish-subscribe operations with an MQTT broker over a plaintext connection\n");
    ...
    let subscribe_packet = SubscribePacket::builder()
        ...
        .build();
    let subscribe_result = client.subscribe(subscribe_packet, None).await;
    ...
    for i in 1..11 {
        ...
        let publish_result = client.publish(publish_packet, None).await;
    ...
    let unsubscribe_packet = UnsubscribePacket::builder()
        ...
        .build();
    let unsubscribe_result = client.unsubscribe(unsubscribe_packet, None).await;
